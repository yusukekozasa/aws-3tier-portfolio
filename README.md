...
### 3. 将来の拡張性を担保したネットワーク分割
検証コストを抑えるため現在はEC2にパブリックIPを付与していますが、コード上ではいつでもプライベートサブネットへ移行できるよう、あらかじめ `private_1` `private_2` のサブネット（`10.0.10.0/24`, `10.0.20.0/24`）を定義し、将来の本格的な3層アーキテクチャへの移行を見据えた設計にしています。
